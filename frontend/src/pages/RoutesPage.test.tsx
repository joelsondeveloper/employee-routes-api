...
describe("RoutesPage", () => {
  ...
  it("sends only selected guest employees to the real optimization motor", async () => {
    const guestEmployee = {id: "demo-01", name: "Lucas Almeida", address: "Centro, Cabo", phone: "(81) 99000-0001", latitude: -8.29, longitude: -35.03};
    const optimizeGuest = vi.spyOn(api, "optimizeGuestRoutes").mockResolvedValue({...resultFixture(), groups: [], issues: [], summary: {...resultFixture().summary, totalEmployees: 1, totalGroups: 0, acceptableGroups: 0, rejectedGroups: 0, unroutableEmployees: 0}});
    const user = userEvent.setup();
    render(<RoutesPage mode="guest" employees={[guestEmployee]} />);

    expect(screen.getByText("Modo visitante")).toBeInTheDocument();
    expect(screen.getByText(/1 funcionário disponível/)).toBeInTheDocument();
